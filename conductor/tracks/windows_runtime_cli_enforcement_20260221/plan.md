...
- [ ] Task: Run `npm run check` and `npm test` in an environment with `pwsh.exe` available.

## Progress Notes
- Runtime enforcement changes are implemented and wired.
- Full command-based verification remains pending due shell tooling constraints in this environment.
