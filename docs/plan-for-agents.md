...
*   **Phase A1: Environment Initialization (Agent 1)**
    ...
    *   [ ] Build the `twinclaw.json` configuration manager to persist settings in `%USERPROFILE%\.twinclaw\workspace\` (replacing `.env`). Supports profile isolation via `TWINCLAW_PROFILE` environment variable.
